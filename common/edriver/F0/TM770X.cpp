...
　　NO.1-  初始版本 基于深圳天威提供的演示程序
	  NO.2-  2017年7月1日 可以驱动TM7707,TM7705,修改文件名为TM770x		
 ...
 *@retval   NONE
*/
TM770X::TM770X(E_SPI *spi,type_T tm_id,E_PinID DRDY,E_PinID CS,E_PinID RST){
	_spi = spi;
	_drdy = (DRDY != P_NC? new E_GPIO(DRDY):NULL);
	_cs =(CS != P_NC? new E_GPIO(CS) :NULL) ;
	_rst =(RST != P_NC?  new E_GPIO(RST) :NULL) ;
	_id =  tm_id;
}
...
{
	_csSet(0);
	_spi->writeChar((dat1>>16) & 0xff);
	_spi->writeChar((dat1>>8) & 0xff);
	_spi->writeChar(dat1 & 0xff);
	_csSet(1);
}

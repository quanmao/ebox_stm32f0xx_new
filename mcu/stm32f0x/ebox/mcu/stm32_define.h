...
/**
 *@brief  GPIO mode相关参数定义，注意:PIN_MODE(@enum)里的顺序必须和E_PinMode_MAP[]
 * 保持一致
 ********************************************************************************/
typedef struct{
	 uint32_t mode;
...
const E_PIN_MODE_T E_PinMode_MAP[] ={
...
__STATIC_INLINE uint8_t PeriphIndex(uint32_t Periphx,const Periph_T *emap)
{
		...
		{
				if((emap+i)->name == (uint16_t)NC){return (uint8_t)NC;}
				i++;
		}
		return i;
}
...
__STATIC_INLINE uint8_t PinIndex(uint16_t pin,const E_PIN_FUN_T *emap)
{
		...
		while(!((emap+i)->id == pin ))
		{
				if((emap+i)->id == (uint16_t)NC){return (uint8_t)NC;}
				i++;
		}
		return i;
}
...
__STATIC_INLINE uint8_t PinAlternate(uint16_t pin,const E_PIN_FUN_T *emap)
{
		while(!((emap->id == pin)))
		{
				if(emap->id == (uint16_t)NC){return (uint8_t)NC;}
				emap++;
		}
...
__STATIC_INLINE uint8_t getIndex(uint8_t pin_id,const Periph_Com_T *emap)
{
		...
		{
				if((emap+i)->pin_id == (uint8_t)NC){return (uint8_t)NC;}
				i++;
		}
		return i;
}
